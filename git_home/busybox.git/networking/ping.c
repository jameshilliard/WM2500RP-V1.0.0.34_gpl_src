...
static unsigned total_secs;    /* record ping command total secs */
static unsigned deadline;  /* for new option -w */
static unsigned timeout;   /* for new option -W */
static unsigned ttmax;	/* tmax * 100 */
...
static void sendping(int junk)
{
	...
	if (pingcount == 0 || deadline || ntransmitted < pingcount) {	/* schedule next in 1s */
		...
		unsigned int expire = timeout;

		if (nreceived) {
			expire = ttmax / (512*1024);
			if (expire == 0)
				expire = 1;
		}
		signal(SIGALRM, pingstats);
...
int ping_main(int argc, char **argv)
{
	char *opt_c, *opt_s, *opt_I, *opt_w, *opt_W;
	...
	opt_complementary = "=1";
	getopt32(argc, argv, OPT_STRING, &opt_c, &opt_s, &opt_I, &opt_w, &opt_W);
...
make it true for ping. */
	...
	if (option_mask32 & 16) deadline = xatoul(opt_w);
	if (option_mask32 & 32) timeout = xatoul(opt_W);
	printf("deadline:%d, timeout:%d\n", deadline, timeout);
	myid = (int16_t) getpid();
	ping(argv[optind]);
	return EXIT_SUCCESS;
}
