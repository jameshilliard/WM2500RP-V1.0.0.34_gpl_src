...
wifitoggle_atheros11n() {
    ...
    if [ "$NEXT_STATE" = "on" ]; then
        num=0
		rm -f /tmp/activatevap
        while [ $num -lt $NUM_RADIO ]; do
			if [ $num = "0" -a "$($DNICONFIG get endis_wl_radio)" = "1" ] || 
				[ $num = "1" -a "$($DNICONFIG get endis_wla_radio)" = "1" -a "$($DNICONFIG get wla_country)" != "14" ]; then
        ...
        while [ $num -lt $MAX_VAP ]; do
            ...
            if [ "x$vap_status" != "x" ]; then
				...
				if [ -n "$is_gband" -a "$($DNICONFIG get endis_wl_radio)" = "1" ] || 
					[ -z "$is_gband" -a "$($DNICONFIG get endis_wla_radio)" = "1" ]; then
					ifconfig ath$num up 2>&1 > /dev/null
					[ ! -f /tmp/activatevap ] && echo -n "ath$num" >> tmp/activatevap || echo -n "|ath$num" >> /tmp/activatevap
				fi
        ...
        if [ $NUM_RADIO -eq 1 ]; then
			[ "$($DNICONFIG get endis_wl_radio)" = "1" ] && [ "${G_SECURITY_TYPE}" -gt "1" ] && eval ${WPS_LED_ON} || eval ${WPS_LED_OFF}
        else
			[ "$($DNICONFIG get endis_wl_radio)" = "1" -o "$($DNICONFIG get endis_wla_radio)" = "1" ] && [ "${G_SECURITY_TYPE}" -gt "1" -o "${A_SECURITY_TYPE}" -gt "1" ] && eval ${WPS_LED_ON} || eval ${WPS_LED_OFF}
        fi
		...
		if [ -f /etc/wpa2/vap_create_only -a -f /tmp/conf_filename ]; then
			...
			if [ "$($DNICONFIG get endis_wl_radio)" = "1" ] || [ "$($DNICONFIG get endis_wla_radio)" = "1" ]; then
				if [ -f /tmp/activatevap ]; then
					cat /tmp/conf_filename | awk -v fn=`cat /tmp/activatevap` '{for (i=1; i<=NF; i++) if ( $i ~ fn) printf "%s ", $i }' > /tmp/enabled_vap
		...
		if [ "$wla_exist" = "on" -a "$($DNICONFIG get endis_wla_radio)" = "1" -a "$($DNICONFIG get wla_country)" != "14" ]; then
            cat /proc/uptime | sed 's/ .*//' > /tmp/WLAN_uptime_5G
            # Turn on wlan LED (5G) if wlan LED is controled by 'ledcontrol'
            [ $WLAN_LED_CONTROL -eq 1 ] && /sbin/ledcontrol -n wlan -c blue -s on
        fi
...
wps_atheros11n() {
    ...
    if [ "${HOSTAPD_VER_PREFIX}" = "v0.7" -o "${HOSTAPD_VER_PREFIX}" = "v0.8" -o "${HOSTAPD_VER_PREFIX}" = "v2.0" ]; then
        if [ "$hwmode" = "dual" ]; then
