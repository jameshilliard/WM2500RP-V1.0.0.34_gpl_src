...
athrs_ar8033_mgmt_init(void)
{
    ...
    rddata |= GPIO_IN_ENABLE3_MII_GE1_MDI_SET(22); //set GE1_MDI to GIP 22
    ath_reg_wr(GPIO_IN_ENABLE3_ADDRESS, rddata);
    //ath_gpio_config_output(ATH_GPIO);
    //ath_gpio_config_output(ATH_GPIO21);
    ath_reg_rmw_clear(GPIO_OE_ADDRESS, ATH_GPIO);

    ath_reg_rmw_clear(GPIO_OE_ADDRESS, ATH_GPIO21);

    rddata = ath_reg_rd(GPIO_OUT_FUNCTION5_ADDRESS) &
             ~(GPIO_FUNCTION5_MASK);
    rddata |=(GPIO_FUNCTION5_ENABLE);
