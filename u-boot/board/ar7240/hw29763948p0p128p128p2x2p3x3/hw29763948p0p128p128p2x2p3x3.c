...
int	checkboard(args)
{
	board_str("U-boot dni29 V1.0 for DNI HW ID: 29763948 flash 128MB RAM 128MB 1st Radio 2x2 2nd Radio 3x3\n");
	return 0;
}
...
/*function do_image_boot_num_set()
 *description:
 * read in image boot num, then call set_board_data() to write to flash.
 * return value: 0 (store in memory A), 1 (store in memory B)
 */
//judge use firmware A or B
int do_image_boot_num_set(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
        u8 image_boot_num[IMAGE_BOOT_NUM_LENGTH + 1];
        ...
        memset(image_boot_num, 0, sizeof(image_boot_num));
        memcpy(image_boot_num, argv[1], IMAGE_BOOT_NUM_LENGTH);

        if ((*image_boot_num != '0') && (*image_boot_num != '1')) {
                printf("Wrong IMAGE BOOT NUM\n");
                return 1;
        }

        printf("Burn image_boot_num (= %s) into ART block\n", image_boot_num);
        set_board_data(IMAGE_BOOT_NUM_OFFSET, IMAGE_BOOT_NUM_LENGTH, image_boot_num);
        puts("Done.\n");
        return 0;
}

U_BOOT_CMD(
        image_boot_num_set, 2, 0, do_image_boot_num_set,
        "image_boot_num_set \t- set image_boot_num\n",
        "Usage:\n"
        " image_boot_num_set X\n"
);

int get_image_num(void)
{
	u8 image_boot_num_buf[IMAGE_BOOT_NUM_LENGTH + 1];

	memset(image_boot_num_buf, 0, sizeof(image_boot_num_buf));

	get_board_data(IMAGE_BOOT_NUM_OFFSET, IMAGE_BOOT_NUM_LENGTH, image_boot_num_buf);

	if(*image_boot_num_buf == '0')
		return 0;
        else if(*image_boot_num_buf == '1')
		return 1;
	else
		return 0; /* return default value 0 (store in memory 0)*/
}

int do_get_image_num(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
        return get_image_num();
}

int do_image_boot_num_show(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	u8 image_boot_num_buf[IMAGE_BOOT_NUM_LENGTH + 1];
	...
	printf("image_boot_num : %s\n", image_boot_num_buf);
        return 0;
}

U_BOOT_CMD(
        image_boot_num_show, 1, 0, do_image_boot_num_show,
        "image_boot_num_show \t- show image_boot_num\n",
        "Usage:\n"
        " image_boot_num_show\n"
        " For instance: image_boot_num_show\n"
);

U_BOOT_CMD(
        image_boot_num_get, 1, 0, do_get_image_num,
        "image_boot_num_get \t- get_image_num\n",
        "Usage:\n"
        " image_boot_num_get\n"
        " For instance: image_boot_num_get\n"
);
