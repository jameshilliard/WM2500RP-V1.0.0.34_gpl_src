...
#define WN2000RPTV3_MODELID "wn2000rptv3"

int power_led_gpio(void);
...
int power_led_gpio()
{
    ...
    if (!strcmp(WN2000RPTV3_MODELID, board_model_id)) {
        printf("%s # %d: modelid is %s\n", __FUNCTION__, __LINE__, board_model_id);
        return 17;
    }
    return POWER_LED_GPIO;
}

int status_led_gpio(void);
...
int status_led_gpio()
{
    ...
    return STATUS_LED_GPIO;

}

int wan_gled_gpio(void);
...
int wan_gled_gpio()
{
    ...
    return WAN_GLED_GPIO;
}

int wan_aled_gpio(void);
...
//	rddata = ar7240_reg_rd(ATH_GPIO_OUT_FUNCTION4);
	if (!strcmp(WN2000RPTV3_MODELID, board_model_id)) {
		ar7240_reg_wr(ATH_GPIO_OUT_FUNCTION4, 0x0000002c);            /* 19, 18, 17, 16 */
    } else {
...
int	checkboard(void)
{
	printf("DNI HW ID: 29763904 flash 4MB RAM 32MB U-boot dni29 V1.0\n");
	return 0;
}
