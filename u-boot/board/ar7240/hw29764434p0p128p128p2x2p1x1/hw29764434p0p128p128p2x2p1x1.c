...
//	ar7240_reg_wr(AR9344_GPIO_OUT_FUNCTION1, (rddata & 0xffffffff)); /* 07, 06, 05, 04 */
	...
	ar7240_reg_wr(AR9344_GPIO_OUT_FUNCTION2, (rddata & 0x00ffffff)); /* 11, 10, 09, 08 */
...
//	ar7240_reg_wr(AR9344_GPIO_OUT_FUNCTION5, (rddata & 0xffffffff)); /* 23, 22, 21, 20 */
	...
	ar7240_reg_rmw_clear(AR9344_GPIO_OE, (1 << WLAN_LED_GPIO)
	                    | (1 << USB_EN_GPIO) | (1 << USB_LED_GPIO)
	                    | (1 << WAN_GREEN_LED_GPIO) | (1 << STATUS_LED_GPIO)
	                    | (1 << WAN_AMBER_LED_GPIO) | (1 << POWER_LED_GPIO)
	                    | (1 << xLNA_0_GPIO) | (1 << xLNA_1_GPIO) );
	/* let gpio pin of reset button be input enabled. */
	ar7240_reg_rmw_set(AR9344_GPIO_OE, (1 << WLAN_ON_OFF_SW_GPIO)
	                    | (1 << USB_FLAG_GPIO) | (1 << WPS_SW_GPIO)
	                    | (1 << RESET_BUTTON_GPIO));
}
...
int	checkboard(args)
{
	board_str("U-boot dni29 V0.3 for DNI HW ID: 29764434 flash 128MB RAM 128MB 1st Radio 2x2 2nd Radio 1x1\n");
	return 0;
}
...
int misc_init_r(void)
{
	...
	ar9344_status_led_on();
	ar9344_power_led_off();
	ar9344_gpio_set(WAN_GREEN_LED_GPIO);
	ar9344_gpio_set(WAN_AMBER_LED_GPIO);
...
int do_ledctl (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (val == 0) {
		...
		ar9344_power_led_on();
		ar9344_gpio_clear(WAN_GREEN_LED_GPIO);
		ar9344_gpio_clear(WLAN_LED_GPIO);
		ar9344_gpio_clear(USB_LED_GPIO);
	} else if(val == 2) {
		ar9344_status_led_on();
		ar9344_gpio_clear(WAN_AMBER_LED_GPIO);
		ar9344_gpio_clear(WLAN_LED_GPIO);
		ar9344_gpio_clear(USB_LED_GPIO);
	}
	return 0;
...
U_BOOT_CMD(
		...
		"ledctl	- Test LEDs\n",
		"\nledctl 1: turn on: power (green) / WAN (green) / WLAN (blue) / USB (blue)\n"
		"\nledctl 2: turn on: status (amber) / WAN (amber) / WLAN (blue) / USB (blue)\n"
