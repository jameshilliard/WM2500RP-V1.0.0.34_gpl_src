...
{    
	...
	if  ( lan_dhcp == 1 && setup_wizard_tag != 1 )
	{
	...
	{
		if(mobile_tag == 1)
			location.href="mobile_ca_apclient_manual.htm";
		else
	...
	else
	{
	    ...
	    if(security=="WPA/WPA2-PSK" || security=="WPA-PSK" || security=="WPA2-PSK")
       {
           if(mobile_tag == 1)
         ...
         if(each_info[1] == "&nbsp;&nbsp;")
            ...
            if(security=="OFF")
            {
              ...
              // }
                ...
                {*/
                   if(mobile_tag == 1)
                       cf.action="/apply.cgi?/mobile_ca_ap_settings.htm timestamp="+ts;
                   else
                       cf.action="/apply.cgi?/ca_ap_settings.htm timestamp="+ts;
               // }
...
//fix bug 27618:"continue" button doesn't work in IE9,so add a tag to instead of "parent.ap_list_reset"
...
{  
	...
	document.forms[0].wlan_ap_wifi.value = xss_replace(document.forms[0].wlan_ap_wifi.value);
	...
	if(tag == 1)
	{
		if(cf.hidden_sec_ap_type.value==0)
		{
			alert("$sec_alert");
			return false;
		}
		else if(cf.hidden_sec_ap_type.value==2)
		{
			if(checkwep_byid(cf)==false)
				return false;
			cf.hidden_sec_ap_type.value=2;
		...
		{        
			...
			else if(cf.hidden_sec_ap_type.value==6)
				cf.ap_sec_wpaphrase.value=getObj("textWpaPwdPhrase4").value;
			else if(cf.hidden_sec_ap_type.value==7)
				cf.ap_sec_wpaphrase.value=getObj("textWpaPwdPhrase5").value;
...
function wladv_apply()
{
	...
	if( wps_progress_status == "1" || wps_progress_status == "9" || wps_progress_status == "10" )
	{
		alert("$wps_in_progress");
		return false;
	}
/*
	...
	if(wps_protect_pin_flag == 1)
	{
		if(cf.pin_enable.checked == true )
			...
			cf.hidden_endis_wps.value="1";
		if(cf.protect_enable.checked == true)
			cf.hid_protect_enable.value="1";
		else
			cf.hid_protect_enable.value="0";

		if( cf.wps_fail_count.value.checkNum())
		{
			alert("$invalid_pin_connections_num");
			cf.wps_fail_count.value.focus();
			return false;
		}
		cf.wps_fail_count.value = parseInt(cf.wps_fail_count.value, 10);
	}
	else
	{
        ...
        if(cf.keep_exist.checked==true)
                cf.hidden_keep_exist.value="disabled";
        else
                cf.hidden_keep_exist.value="enabled";

	if(cf.endis_ssid_broadcast.checked == false)
	{
...
/*			var ap_transmit=cf.hidden_enable_ap_transmit.value;
		...
		{		
			...
			else if(getObj("type_id7").checked == true)
			{
				...
				cf.hidden_wpa_psk.value = getObj("textWpaPwdPhrase4").value;
			}
			else
	...
	if(cf.hidden_sec_type.value == 1)
	{
		if(!confirm("$wps_warning3"))
			return false;
	}
	if(cf.hidden_sec_type.value == 2 || cf.hidden_sec_type.value == 3)
	{
...
function check_ca_wlan()
{
	...
	else
	{
		...
		{
			cf.hidden_sec_type.value=4;
			cf.hidden_wpa_psk.value = get_wpa_aes;
		}
		else if(old_sectype == 5)
		{
			cf.hidden_sec_type.value=6;
			cf.hidden_wpa_psk.value = get_wpa2_tkip;
		}
		else if(old_sectype == 6)
		{
			cf.hidden_sec_type.value=6;
			cf.hidden_wpa_psk.value = get_wpa2;
		}
		else if(old_sectype == 7)
		{
			cf.hidden_sec_type.value=7;
	...
	if(mobile_tag == 1)
		alert("$mobile_ca_change" + cf.hidden_wlan_ssid.value + "$mobile_ca_follow1" + "$mobile_ca_follow2" + "$mobile_ca_follow3")
