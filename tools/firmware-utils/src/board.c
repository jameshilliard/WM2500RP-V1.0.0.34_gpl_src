/*
 * board.c - partially based on OpenWrt's add_header.c
 *
...
static void usage(const char *mess)
{
	...
	fprintf(stderr, "Usage: [board] input_file output_file magic_number\n");
	fprintf(stderr, "\n");
	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	printf("The magic_number is %s!\n", argv[3]);
        header.magic = htonl(strtol(argv[3], NULL, 16)); /* board in ascii */
	header.crc = 0;
